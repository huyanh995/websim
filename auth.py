...
def get_alpha_info(alpha_id, sess):
    ...
    try:
        if ERRORS(sess, response.content):
            ...
            alpha_info["create_day"]=int(alpha_res_json["dateCreated"].split("T")[0].replace("-",""))
...
def check_submission(alpha_id, sess): # Need to check again, exception at b' ' << Check later. THROTTLED << Check its meaning.
...
alpha_id = "JRW0Kvn"
for x, y in get_alpha_info("LO78Gv6",sess).items():
    print(str(x)+" : "+str(type(y)))
# for x in range(101):
